...
const uint64 kBackBase = 0x2f2bc20000ull;	// I just hope this is allowed
...
static uint32* back_data;
static uint32* back_pos;
static int back_size;
static void mmap_output(int size);
static void mmap_back(int size);
static uint32* write_output(uint32 v, bool back = false, char *mem = NULL, int sz = 0);
static uint32* write_output_64(uint64 v, bool back = false, char *mem = NULL, int sz = 0);
static void write_completed(uint32 completed, bool back = false);
...
static void mmap_back(int size)
{
	if (size <= back_size)
		return;
	if (size % SYZ_PAGE_SIZE != 0)
		failmsg("trying to mmap backmem area that is not divisible by page size", "page=%d,area=%d", SYZ_PAGE_SIZE, size);
	uint32* mmap_at = NULL;
	if (back_data == NULL) {
		// It's the first time we map output region - generate its location.
		back_data = mmap_at = (uint32*)(kBackBase + (1 << 20) * (getpid() % 128));
	} else {
		// We are expanding the mmapped region. Adjust the parameters to avoid mmapping already
		// mmapped area as much as possible.
		// There exists a mremap call that could have helped, but it's purely Linux-specific.
		mmap_at = (uint32*)((char*)(back_data) + back_size);
	}
	void* result = mmap(mmap_at, size - back_size,
			    PROT_READ | PROT_WRITE, MAP_SHARED | MAP_FIXED, kBackFd, back_size);
	if (result != mmap_at)
		failmsg("mmap of back file failed", "want %p, got %p", mmap_at, result);
	back_size = size;
}
#endif
...
void receive_execute()
{
	...
	flag_priv_mode = (req.exec_flags >> 6) & 0x07;
...
void execute_one()
{
	...
	/*
		...
		2. CAP_PERFMON
		3. CAP_NET_ADMIN
		4. CAP_SYS_ADMIN: CAP_BPF + CAP_PERFMON + CAP_NET_ADMIN
	*/
	cap_value_t cap_level4[]    =   {CAP_SYS_ADMIN}, // {CAP_BPF, CAP_PERFMON},
				cap_level3[]	=	{CAP_NET_ADMIN},
	            cap_level2[]    =   {CAP_PERFMON},
	            cap_level1[]    =   {CAP_BPF};

	fprintf(stderr, "flag_priv_mode: %llu\n", flag_priv_mode);
	cap_t caps = cap_get_proc();
	if (caps == NULL) {
		fail("cap_get_proc");
	}
	if (cap_set_flag(caps, CAP_PERMITTED, 1, cap_level3, CAP_SET) != 0 ||
		cap_clear_flag(caps, CAP_EFFECTIVE) != 0 ||
		cap_set_proc(caps) != 0) {
		fail("Set all caps as permitted and clear effective caps failed\n");
	}

	cap_value_t *cur_cap_level = NULL;
	int size = 0;
	switch (flag_priv_mode) {
		case 0:
			break;
		case 1:
			cur_cap_level = cap_level1;
			size = sizeof(cap_level1)/sizeof(cap_value_t);
			break;
		case 2:
			cur_cap_level = cap_level2;
			size = sizeof(cap_level2)/sizeof(cap_value_t);
			break;
		case 3:
			cur_cap_level = cap_level3;
			size = sizeof(cap_level3)/sizeof(cap_value_t);
			break;
		case 4:
			cur_cap_level = cap_level4;
			size = sizeof(cap_level4)/sizeof(cap_value_t);
			break;
		default:
			fail("Incorrect capability mode\n");
	}

	if (flag_priv_mode != 0) {
		if (cap_set_flag(caps, CAP_EFFECTIVE, size, cur_cap_level, CAP_SET) != 0 ||
			cap_set_proc(caps) != 0) {
			fail("Fail to set capabilties effective\n");
		}
	}
...
void write_call_output(thread_t* th, bool finished)
{
	...
	// Only write return values and logs after finishing BPF_PROG_LOAD 
	if (finished && th->args[0] == BPF_PROG_LOAD) {
		write_output_64(th->res, true);
		int sz = (strlen((const char *)(((union bpf_attr *)(th->args[1]))->log_buf)) & 0xffffffff);
		write_output(sz, true);
		write_output(0, true, (char *)((union bpf_attr *)(th->args[1]))->log_buf, sz);
	}
...
#if SYZ_EXECUTOR_USES_SHMEM
uint32* write_output(uint32 v, bool back, char *mem, int sz)
{
	if (back) {
		if (back_pos < back_data || (char*)back_pos >= (char*)back_data + back_size)
		failmsg("backmem overflow", "pos=%p region=[%p:%p]",
			back_pos, back_data, (char*)back_data + back_size);
		...
		if (sz == 0) {
			*back_pos = v;
			...
			sz = sz + (4-(sz%4));
			int units = sz / 4;
			memcpy((char *)back_pos, mem, sz);
			uint32* old_pos = (uint32*)back_pos + (units-1);
			back_pos += units;
			return old_pos;
		}
	}
...
uint32* write_output_64(uint64 v, bool back, char *mem, int sz)
{
	if (back) {
		if (back_pos < back_data || (char*)(back_pos + 1) >= (char*)back_data + back_size)
		failmsg("backmem overflow", "pos=%p region=[%p:%p]",
			back_pos, back_data, (char*)back_data + back_size);
		if (sz == 0) {
			*(uint64*)back_pos = v;
			uint32* old_pos = (uint32*)back_pos + 1;
			back_pos += 2;
			return old_pos;
		} else {
			sz = sz + (4-(sz%4));
...
void write_completed(uint32 completed, bool back)
{
	if (back)
		__atomic_store_n(back_data, completed, __ATOMIC_RELEASE);
	else
