...
func (mon *monitor) monitorExecution() *report.Report {
	...
	for {
		...
		case out, ok := <-mon.outc:
			...
			// Specifically process semantic bug reports here
			// "ebpf-fuzz semantic checker [differential testing]"
			if bytes.Contains(mon.output[mon.matchPos:], []byte("ebpf-fuzz semantic checker")) {
				// log.Logf(0, "ebpf-fuzz semantic checker in manger\n")
				crashdir := filepath.Join(mon.inst.workdir, "../crashes")
				for i := 0; ; i++ {
					filename := filepath.Join(crashdir, "z-crash-log-"+fmt.Sprint(i))
					if !osutil.IsExist(filename) {
						f, err := os.Create(filename)
						if err != nil {
							log.Logf(0, "create file failed: %v", err)
						} else {
							f.Write(mon.output[mon.matchPos:])
							//osutil.WriteFile(filename, mon.output[mon.matchPos:])
						}
						break
					}
				}
