...
func RunManager(cfg *mgrconfig.Config) {
    ...
    if _, err := osutil.Run(10*time.Minute, cmd); err != nil {
        log.Logf(0, "failed to copy spec to /tmp: %v", err)
    }
...
func (mgr *Manager) runInstanceInner(index int, instanceName string) (*report.Report, []byte, error) {
	...
	// Copy the bpf smt spec
	/*
	ex, err := os.Executable()
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get source file path", err)
	}
	bpfSMTSpec := filepath.Dir(ex) + "/../equiv-test/alu-mem-parse.smt2"
	_, err = inst.Copy(bpfSMTSpec)
	if err != nil {
		return nil, nil, fmt.Errorf("failed to copy bpf smt spec", err)
	}
	*/
